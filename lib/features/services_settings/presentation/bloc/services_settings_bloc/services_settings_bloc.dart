...
class ServicesSettingsBloc
  ...
  ServicesSettingsBloc(this._servicesSettingsUseCase)
    ...
    on<GetServicesSettingsEvent>((event, emit) async {
      final branchId = await StorageHelper.getBranchId();
      return _onGetServicesSettings(
  ...
  Future<void> _onGetServicesSettings(
    ...
    result.fold((error) => emit(ServicesSettingsError(error)), (settings) {
  ...
  Future<void> _onUpdateServiceStatus(
    ...
    final updatedServices =
        _currentSettings!.services!.map((service) {
